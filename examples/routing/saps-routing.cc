...
NS_LOG_COMPONENT_DEFINE ("SapsRouterTest");
...
main (int argc, char *argv[])
{
  ...
  // SAPS bouncing switch configuration
  Ipv4SapsHelper saps;
  Ptr<Ipv4> ipv4B = nB->GetObject<Ipv4> ();
  Ptr<Ipv4Saps> ipv4SapsB = saps.GetIpv4Saps(ipv4B);
  ipv4SapsB->AddCoreSwitchAddress(iBiC.GetAddress (1));
  ipv4SapsB->AddCoreSwitchAddress(iBiD.GetAddress (1));

  Ptr<Ipv4> ipv4E = nE->GetObject<Ipv4> ();
  Ptr<Ipv4Saps> ipv4SapsE = saps.GetIpv4Saps(ipv4E);
  ipv4SapsE->AddCoreSwitchAddress(iEiC.GetAddress (1));
  ipv4SapsE->AddCoreSwitchAddress(iEiD.GetAddress (1));
